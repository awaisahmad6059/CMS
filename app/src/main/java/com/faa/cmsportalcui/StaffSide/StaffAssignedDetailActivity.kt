...
class StaffAssignedDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val startTaskButton = findViewById<Button>(R.id.startTask)

        if (!assignedTaskId.isNullOrEmpty()) {
            firestore.collection("assignTasks").document(assignedTaskId)
                .get()
                .addOnSuccessListener { document ->
                    if (document.exists() && document.getBoolean("started") == true) {
                        startTaskButton.visibility = Button.GONE
                    }
                }
                .addOnFailureListener { e ->
                    Log.e("StaffAssignedDetailActivity", "Error checking start status", e)
                }
        }
        ...
        startTaskButton.setOnClickListener {
            if (!assignedTaskId.isNullOrEmpty()) {
                firestore.collection("assignTasks").document(assignedTaskId)
                    .set(mapOf("started" to true), SetOptions.merge())
                    .addOnSuccessListener {
                        Toast.makeText(this, "Task started", Toast.LENGTH_SHORT).show()
                        startTaskButton.visibility = View.GONE
                    }
                    .addOnFailureListener { e ->
                        Toast.makeText(this, "Failed to start task: ${e.message}", Toast.LENGTH_SHORT).show()
                        Log.e("StaffAssignedDetailActivity", "Failed to start task", e)
                    }
            }
        }
