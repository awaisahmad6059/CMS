...
class LoginActivity : AppCompatActivity() {
    ...
    private fun checkUserOrStaff(userId: String, email: String) {
        firestore.collection("staff").document(userId).get()
            ...
            .addOnFailureListener {
                Toast.makeText(this, "Error checking staff info", Toast.LENGTH_SHORT).show()
            }
    }
        ...
        firestore.collection("staff").whereEqualTo("email", email).get()
            .addOnSuccessListener { querySnapshot ->
                if (!querySnapshot.isEmpty) {
                    ...
                    if (userType == "staff") {
                        val randomStaffId = generateRandomStaffId()
                        val staffData = hashMapOf(
                            "email" to email,
                            "name" to username,
                            "userType" to userType
                        )
                        firestore.collection("staff").document(randomStaffId)
                            .set(staffData)
                            ...
                            .addOnFailureListener {
                                Toast.makeText(
                                    this,
                                    "Failed to save staff: ${it.message}",
                                    Toast.LENGTH_SHORT
                                ).show()
                            }
                    } else {
                        Toast.makeText(this, "User not found in any role", Toast.LENGTH_SHORT)
                            .show()
                    }
                }
            }
    ...
    private fun saveUserIfNotExist(email: String) {
        val username = prefs.getString("username", "") ?: ""
        val userType = prefs.getString("userType", "") ?: "user" // default to user
